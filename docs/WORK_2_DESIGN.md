...
为降低横向控制面的固定成本，编排器现在在仿真启动前预编译无变化的任务 dispatch 描述，并缓存
batch 的 schema/run-id JSON 前缀；运行时只编码序号、逻辑时间、microstep 和动态事件数组。事件日志
直接写入已存在的结构化 batch，避免“编码后立即解码”的额外往返。完成时间、重试、frontier 和
